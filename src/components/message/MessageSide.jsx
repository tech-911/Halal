...
const MessageSide = () => {
  ...
  function openMesage(thedata) {
    // console.log(thedata)
    dispatch(
      messageSliceAction({ userData: thedata })
    );
  }
            ...
            {theData ? (
              <>
                {theData.map((thelist, i) => (
                  <div className="mt-5 w-full bg-white flex items-center gap-x-3 shadow-sm p-2 cursor-pointer" onClick={() => openMesage(thelist)} key={i}>
                    <div className="">
                      <img className="w-[50px] h-[50px] object-cover rounded-full" src={thelist.photo[0]} alt="" />
                    </div>
                    <div className="flex justify-between w-full">
                      <div className="">
                        <p className="font-bold text-[14px]">{thelist.name}</p>
                        <p className="text-[14px] text-[#8E8E93] w-9/12">{thelist.name} like you.</p>
